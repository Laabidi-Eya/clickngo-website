...
$sql = "SELECT id, name, description, price, location, image FROM activities ORDER BY created_at DESC LIMIT 50";
...
<body>
  ...
  <button class="back-button" onclick="window.location.href='http://localhost/Projet%20Web/mvcact/view/front%20office/activite.php'">
  ...
  <div class="tinder">
    ...
    <div class="tinder--cards">
      <?php while ($row = $result->fetch_assoc()): ?>
        <div class="tinder--card" data-id="<?= htmlspecialchars($row['id']) ?>" style="background-image: url('<?= htmlspecialchars($row['image']) ?>');">
          <div class="card-overlay">
            <h3><?= htmlspecialchars($row['name']) ?></h3>
            <p><?= htmlspecialchars($row['description']) ?></p>
            <p><strong>📍 <?= htmlspecialchars($row['location']) ?></strong></p>
...
">
      <h2 style="color: #fff; font-size: 24px; margin-bottom: 20px;">
        😅 Il n'y a plus d'activités pour le moment
      </h2>
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      function handleChoice(isLike) {
        ...
        if (isLike) {
          ...
          setTimeout(function() {
            window.location.href = 'http://localhost/Projet%20Web/mvcact/view/front%20office/reservation.php?id=' + activityId;
          }, 500);
        } else {
